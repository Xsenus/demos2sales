...
- Смета демонстрации фиксирована для всех демонстраций и разбита на водительскую часть и прочие расходы ООО ИРБИСТЕХ.
- Поле `Время работы демонстратором` влияет на расчет водителя, демонстратора и криобластера.
- Вычет демо считается по формуле `[VIC] = [K2] × [K1] × [DEMO_COST]`, где `[K2] = [xP] × [xR] × [xM]`.
...
- Справочник товаров поддерживает Excel-экспорт/импорт и офисные параметры `MR/PR/ST`.
