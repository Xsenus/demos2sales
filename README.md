...
- Вычет демо приведен к методике Excel: `[K2] = 0,45 × [xP] + 0,35 × [xR] + 0,20 × [xM]`; `SOFT_STOP` и `HARD_STOP` формируют итоговый `[K3]`; `Уменьшение премии / Вычет [VIC], NET руб. = [DEMO_COST] × [K3] × [СТМ]`. Критерии P/R/M перенесены из Excel-листов `1_Подготовка`, `2_Результат`, `3_Управленческий`.
...
## Обновление дизайна v6

Frontend переведен на единый enterprise/SaaS-каркас по требованиям дизайна интерфейса:

- темное левое меню с бренд-блоком, навигацией и карточкой пользователя;
- верхняя полупрозрачная панель с названием текущего раздела и быстрым обновлением из БД;
- светлая рабочая область с полупрозрачными панелями, крупными скруглениями и мягкими тенями;
- единые CSS-токены в `frontend/src/styles.css`: цвета, радиусы, тени, sidebar/topbar, статусы;
- обновлены кнопки, поля ввода, таблицы, KPI-карточки, бейджи, вкладки, подсказки и адаптивность.

Дополнительно в релизе обновлена методика расчета вычета демонстрации по Excel: добавлены `[K3]`, `SOFT_STOP`, `HARD_STOP` и расчет `[VIC]` через ставку премии от маржи `[СТМ]`.
