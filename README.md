...
- В карточку демонстрации добавлена загрузка одного файла отчета любого формата. Файл хранится в `DEMO_CALC_REPORTS_DIR`, метаданные — в `actions.payload.demo_report`.
- После загрузки отчета доступны кнопки удаления и скачивания; удаление и повторная загрузка блокируются после подтверждения демонстрации.
- В смете демонстрации строка «Амортизация Газели и ТО» разрешает менеджеру корректировать количество.
- Ставка «Работа демонстратора*» вынесена в офисные бизнес-настройки по филиалам и используется в Части 1 сметы.
- В критериях P/R/M отображаются метки `[SOFT_STOP]` и `[HARD_STOP]`.
- В справочнике товаров добавлена настройка ширины колонок, а кнопки перемещения отображаются в одной строке.
- Левое меню можно свернуть, пользовательская сессия сохраняется в `localStorage`.
- Для существующей базы добавлен безопасный скрипт `db/04_migrate_to_v7.sql`; полный дамп заказчика сохранен в `public.sql`.
